...
public class NQueensDemo {
	...
	private static Problem<NQueensBoard, QueenAction> problema = null;
	private static double[] maxQueueSize = {0,0,0,0};
	private static double[] pathCost = {0,0,0,0};
	private static double[] nodesExpanded = {0,0,0,0};
	...
	private static void startNQueensDemo() {
		...
		for(int i=0;i<32;i++) {
			problema = NQueensFunctions.createCompleteStateFormulationProblem(boardSize, Config.QUEEN_IN_EVERY_COL);
			System.out.println("\n-----------"+i+"-----------");
			cont = 0;
	...
	private static void solveNQueensWithBreadthFirstSearch() {
		...
		Problem<NQueensBoard, QueenAction> problem = problema;
	...
	private static void solveNQueensWithUniformCostSearch() {
		...
		atualizarMetricas(search);
		System.out.println(search.getMetrics());
		//maxQueueSize[0] = Double.parseDouble(search.getMetrics().get("nodexExpanded"));
	}
	...
	private static void solveNQueensWithRecursiveDLS() {
		System.out.println("\n--- DepthLimitedSearch recursiva (limite = boardsize = 8)---");
	...
	private static void atualizarMetricas(SearchForActions<NQueensBoard, QueenAction> properties) {
		System.out.println(properties.getMetrics().get("maxQueueSize"));
		System.out.println(properties.getMetrics().get("pathCost"));
		System.out.println(properties.getMetrics().get("nodesExpanded"));
		
		if(cont == 0){
			String mqs = (String)properties.getMetrics().get("maxQueueSize");
			maxQueueSize[cont] += Double.parseDouble(mqs);
			}
		
		String pc = (String)properties.getMetrics().get("pathCost");
		pathCost[cont] += Double.parseDouble(pc);
		
		String ne = (String)properties.getMetrics().get("nodesExpanded");
		nodesExpanded[cont] += Double.parseDouble(ne);
		
		System.out.println("Somatorio MaxQSize: "+ maxQueueSize[cont]);
